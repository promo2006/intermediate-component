...
(function(obj, location, navigator, document, window) {
    ...
    var FORM_PROCESS_URL = "/public/form/process";
    ...
    var duplicatedContactMessage = obj.formDuplicatedContactMessage;
    var firstnameRequired = !!(obj.formFirstnameRequired && obj.formFirstnameRequired.toString() === "true");
    var missingFirstnameMessage = obj.formMissingFirstnameMessage;
    var lastnameRequired = !!(obj.formLastnameRequired && obj.formLastnameRequired.toString() === "true");
    var missingLastnameMessage = obj.formMissingLastnameMessage;
    var emailRequired = !!(obj.formEmailRequired && obj.formEmailRequired.toString() === "true");
    var missingEmailMessage = obj.formMissingEmailMessage;
    var invalidEmailMessage = obj.formInvalidEmailMessage;
    var phoneRequired = !!(obj.formPhoneRequired && obj.formPhoneRequired.toString() === "true");
    var minPhoneDigits = obj.formMinPhoneDigits;
    var maxPhoneDigits = obj.formMaxPhoneDigits;
    var missingPhoneMessage = obj.formMissingPhoneMessage;
    var invalidPhoneMessage = obj.formInvalidPhoneMessage;
    var acceptTermsMessage = obj.formAcceptTermsMessage;
    var redirectToThankyouPage = !!(obj.formRedirectToThankyouPage && obj.formRedirectToThankyouPage.toString() === "true");
    var redirectToThankyouPageOnDuplicate = !!(obj.formRedirectToThankyouPageOnDuplicate && obj.formRedirectToThankyouPageOnDuplicate.toString() === "true");
    var successMessage = obj.formSuccessMessage;
    var conversionCustomCode = obj.formConversionCustomCode;
    // Obtengo variables de mensajes genericos
    var successHeaderText = obj.formSuccessHeaderText;
    var failedHeaderText = obj.formFailedHeaderText;
    var closeButtonText = obj.formCloseButtonText;
    var failedMessage = obj.formFailedMessage;
    var errorMessage = obj.formErrorMessage;
    var timeoutMessage = obj.formTimeoutMessage;
    ...
    function initializeForm(jsLib) {

        // Funcion de submit para el formulario de Form
        var onSubmit = function(event) {
            try {
                // Si el flag esta habilitado no sigo procesando
                if (obj.disableForm) return;
                // Obtengo el objeto form
                var form = jsLib(this);
                // Valido si estan todos los datos requeridos
                if (form.valid() && !formProcessing) {
                    // Muestro el spinner
                    showSpinner(true);
                    ...
                    jsLib.ajax({
                        type: "POST",
                        url: currentProtocol + "://" + currentDomain + (currentBasePath ? currentBasePath : "") + FORM_PROCESS_URL,
        ...
        // Funcion de submit para el formulario de form desde popup (dentro de iframe)
        ...
        function findFormForms() {
            ...
            pageForms.each(function(formIndex, form) {
                // Obtengo atributo para deshabilitar form en este formulario
                var noForm = jsLib(form).attr("data-ic-noform");
                // Si tengo el atributo noForm me salteo el formulario
                if (noForm === "1" || noForm === "true") return true;
                // Obtengo atributo para forzar el canal form en este formulario
                var forceForm = jsLib(form).attr("data-ic-form");
                // Si tengo el atributo forceForm ya agrego el form sin verificar los campos
                if (forceForm === "1" || forceForm === "true") {
                ...
                jsLib(form).find(":input").each(function(index, element) {
                    // Si encuentro un campo para ingresar el telefono (con id o name que contenga firstname, lastname, email, phone o telefono) asumo que es para form
                    if (element.id.toLowerCase().indexOf("firstname") > -1 || element.name.toLowerCase().indexOf("firstname") > -1 ||
                        element.id.toLowerCase().indexOf("lastname") > -1 || element.name.toLowerCase().indexOf("lastname") > -1 ||
                        element.id.toLowerCase().indexOf("email") > -1 || element.name.toLowerCase().indexOf("email") > -1 ||
                        element.id.toLowerCase().indexOf("phone") > -1 || element.name.toLowerCase().indexOf("phone") > -1 ||
                        element.id.toLowerCase().indexOf("telefono") > -1 || element.name.toLowerCase().indexOf("telefono") > -1) {
                        // Si el formulario ya esta agregado dentro de los de C2C (procesados antes) lo salteo
                        if (obj.c2cForms && obj.c2cForms.find(function(formData) {
                            return formData.formId === form.id;
                        })) return;
                        // Agrego el form al array de elegidos
                        chosenForms.push(jsLib(form));
        ...
        // Publico funcion asociada al objeto para poder recibir Form desde iframes
        obj.iframeToForm = function(iframeData) {
            try {
                // Si el flag esta habilitado no sigo procesando
                if (obj.disableForm) return;
                // Muestro el spinner
        ...
        jsLib(document).ready(function() {
            ...
            chosenForms.forEach(function(form, index) {
                ...
                var formId = form.attr("id") || "form_form_" + index.toString();
                ...
                obj.formForms.push(formData);
                // Asocio la funcion de submit definida
                form.submit(isPopup || isInternalPopup ? onSubmitPopup : onSubmit);
                // Al terminar de procesar el ultimo elemento doy por inicializado el canal de form
                if (index === chosenForms.length - 1) {
                    // Paso el flag a true
                    obj.formInitialized = true;
    ...
    function whenJsLibAvailable(callback) {
        ...
        var checkLoop = function() {
            ...
            if (obj && obj.jsLib) {
                ...
                console.error("Shared library failed to load... Form scripts wont initialize");
            }
        };
        window.setTimeout(checkLoop, interval);
    }
