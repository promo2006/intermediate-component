...
import { APP_ASSETS, APP_CLIENT_SRC, BROWSER_LIST, CSS_SRC, ENV, DEPENDENCIES, ENABLE_SCSS, COLOR_GUARD_WHITE_LIST, SASS_LINT_OPTIONS } from '../config';
...
function lintComponentStylesheets() {
  return gulp.src([
    ...
    `!${join(CSS_SRC, '**', '*.css')}`
  ]).pipe(isProd ? plugins.cached('css-lint') : noop())
    .pipe(ENABLE_SCSS ? plugins.sassLint(SASS_LINT_OPTIONS) : plugins.postcss(processors))
    .pipe(ENABLE_SCSS ? plugins.sassLint.format() : noop())
    .pipe(ENABLE_SCSS ? plugins.sassLint.failOnError() : noop());
}
